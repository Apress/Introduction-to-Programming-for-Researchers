...
'''
'''
this template uses a per_gc_content (module or script) and a function we created in class to aid comprehension.
name your actual unit test script "test_<per_gc_content WE'RE TESTING>.py"
'''

# 1. get the function to test or import the module to test.
from my_module import per_gc_content

# 2. create a class into which we pass the unit tests we want to perform.
class Test_per_gc_content(unittest.TestCase):

   # 3. create a method for each garbage_filter test to perform.    
   def test_per_gc_content_GF_1(self):
   ...
   # 3. create a method for each garbage_filter test to perform.    
   def test_per_gc_content_GF_2(self):
        ''' assert input must be a clean nucleotide string
        '''
        with self.assertRaises(AssertionError):
            per_gc_content('gattaxa')   

   # 4. create a method for each test to perform.
   def test_per_gc_content(self):
       result = per_gc_content('gattacagattaca')
